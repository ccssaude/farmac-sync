...
if(!is.logical(con_farmac) & !is.logical(con_local)){
  ...
  if(exists('pacientes_farmac') && exists('pacientes_referidos')){
    ...
    if (nrow(pacientes_farmac)>0 & nrow(pacientes_referidos)>0)
    {
      novos_pacientes_por_inserir_farmac = anti_join(pacientes_referidos, pacientes_farmac, by='id')
    }
    else {
      
      novos_pacientes_por_inserir_farmac = pacientes_referidos
    }
    ...
    if(dim(novos_pacientes_por_inserir_farmac)[1] > 0){
      ...
      status = receberPacienteServer(con_local,novos_pacientes_por_inserir_farmac )
      
      ##dbWriteTable(con_local, 'sync_temp_patients', novos_pacientes_por_inserir_farmac, append=TRUE, row.names=FALSE)
      ...
      if(status){
        ...
        for (i in 1:nrow(novos_pacientes_por_inserir_farmac)){
          ...
          load(file='config\\patient.RData')
          ...
          id = last_id_pi +5
          patient_id = last_id+5
          value  = novos_pacientes_por_inserir_farmac$patientid[i]
          type_id = 0
          
          load(file='config\\patientidentifier.RData')
          ...
          load(file='config\\patientattribute.RData')
          patientiattribute = add_row(patientiattribute, id = id, value =value_att, patient=patient_att, type_id =    type_id_attribute )
          ...
          status=insertPatient(con_local, patient)
          if (status){
            print (paste0('paciente nid: ',patientid, '-', firstnames, 'inserido com sucesso' ))
            dbExecute(con_farmac,paste0("update sync_temp_patients set imported = 'yes' where patientid= '",patientid, "' ;" ))
          }
          
          insertPatientIdentifier(con_local, patientidentifier)
          insertPatientAttribute(con_local, patientiattribute)
