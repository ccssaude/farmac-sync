...
if(!is.logical(con_local)){
  ...
  dbExecute(con_local, paste0(" update sync_temp_dispense set imported = 'yes', openmrs_status='yes', send_openmrs ='yes'
                               where dispensedate::date > '" ,today , "' ;"))
      ...
      if(nrow(all_patient_dispenses_dups)==0){
        ...
        if(patient_id==0){
          ...
          saveLogError(us.name = main_clinic_name,
                       event.date = Sys.Date(),
                       action = 'Obter o patientid da tabela Patient',
                       error = paste0('Not Found patientid', nid))
